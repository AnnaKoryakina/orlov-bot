...
# ----------------- НАСТРОЙКИ ХАРАКТЕРА -----------------
SARCASM_LEVEL = 2          # 0..3 — чем выше, тем ехиднее
MAX_WRONG_STREAK = 3       # после N подряд неверных — лёгкий "укол"

# ----------------- РЕПЛИКИ -----------------
# Бытовые/шутливые авто-реплики (15–30 минут)
IDLE_REPLIES = [
    # Серьёзные
...
# Варианты ответов на неверный ввод (серьёзные + юмор)
WRONG_REPLIES = [
    # Серьёзные
    ...
    "Давай так: ты — ещё раз пробуешь, я — делаю вид, что удивлён.",
]

# Редкие длинные атмосферные (10% шанс)
LONG_WRONG_REPLIES = [
...
SPECIAL_FUNNY = {
    "самолет": "Самолёт? Только если кукурузник. Остальное — роскошь, не по легенде ✈️🌽",
    "торт": "Торт — это святое. Но уликам нужнее крошки, чем нам калории 🎂",
    "не знаю": "Фраза, достойная отчёта. Но давай попробуем «узнаю» 😉",
    "нужна помощь": "Помощь уже выехала… на лифте. Может, пока сам справишься?",
...
async def send_idle_message(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    delay = random.randint(900, 1800)  # 15–30 мин
...
# Follow-up после «тайное свидание» (20 мин)
...
# КОФЕ: ежедневный пинг в 11:47 МСК
